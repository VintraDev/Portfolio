    ...
    const cardsData = [
        {
            ...
            description: 'Um site de e-commerce para o lançamento do iPhone 17 Pro Max, focado em design moderno e experiência de usuário.',
            category: ['front-end'],
        ...
        },
        {
            ...
            description: 'Um site para a Clinica Sensory, focado em acessibilidade e experiência de usuário, utilizando as melhores práticas de desenvolvimento web.',
            category: ['front-end'],
        ...
        },
        {
            ...
            description: 'Uma aplicação de análise de feedbacks, focada em arquitetura de software escalável e experiência de usuário intuitiva.',
        ...
        },
        {
            ...
            description: 'Um convite digital para um aniversário, focado em design criativo e experiência de usuário envolvente.',
        ...
        <section className="px-6 md:px-10 lg:px-20 pt-10 bg-gray-50 lg:pt-30">
            <div>
                ...
                <p className="text-(--tertiary-color)/90 mb-6 font-work-sans">
                    Uma demonstração dos meus trabalhos, focando em arquitetura de software, escalabilidade e experiência de usuário.
                </p>
                ...
                <div data-dropdown className="relative w-max">
                    ...
                    >
                        ...
                        <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M3 4h18l-7 7v6l-4 2v-8L3 4z" />
                        </svg>
                        <span className="text-white/60 font-normal">Filtrar por:</span>
                        <span>{filters.find(f => f.value === activeFilter)?.label ?? "Todos"}</span>
                        <svg
                    ...
                    {dropdownOpen && (
                        <ul className="absolute top-full mt-2 left-0 w-full z-50 bg-white rounded-2xl shadow-[0_8px_24px_rgba(0,0,0,0.12)] overflow-hidden border border-gray-100">
                            {filters.map((filter, index) => (
                                <li key={index}>
                                    <button
                                        onClick={() => { setActiveFilter(filter.value); setDropdownOpen(false); }}
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                        <Cards items={filteredCards} className='flex-col' renderContent={(item) => (
                            <>
                                <motion.a
                                    initial="hidden"
                                    whileHover="visible"
                                ...
                                >
                                    <img className="absolute inset-0 size-full shadow-md object-cover border border-white rounded-tr-md rounded-tl-md" src={item.image} alt={item.title} />
                                    <motion.div
                                ...
                                <div className="p-4 flex flex-col gap-2">
                                    <div className="flex flex-row gap-2 items-center justify-center">
                                        {item.skills.map((skill, index) => (
                                            <span key={index} className="bg-(--primary-color) text-white text-xs font-medium px-2 py-1 rounded w-full text-center">
                                                {skill}
                                            </span>
                                        ))}
                                    </div>
                                    ...
                                    <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-neutral-600 font-work-sans hover:underline block">
                                        Visualizar Projeto
                                    </a>
                                </div>
                            </>
